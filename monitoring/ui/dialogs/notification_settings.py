...
from tkinter import (
    ...
    Button, ACTIVE, Listbox, END, SINGLE
)
...
class NotificationSettingsDialog(Dialog):
    ...
    def body(self, master: Frame) -> Frame:
        ...
        Label(master, text="Destinataire:").grid(row=5, column=0, sticky="e", padx=5, pady=4)

        rcpt_row = Frame(master)
        rcpt_row.grid(row=5, column=1, sticky="we", padx=5, pady=2)
        ...
        Label(master, text="Liste:").grid(row=6, column=0, sticky="ne", padx=5, pady=4)
        self.lst_recipients = Listbox(master, width=32, height=6, selectmode=SINGLE)
        self.lst_recipients.grid(row=6, column=1, sticky="we", padx=5, pady=(2, 4))
        ...
        ).grid(row=7, column=0, columnspan=2, sticky="w", padx=5, pady=(2, 4))
    ...
    def _on_test(self) -> None:
        """Envoie un email de test avec les paramètres saisis."""
        try:
            send_alert_email(
                "Test notification",
